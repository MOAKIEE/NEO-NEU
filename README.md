...
正式 App 已集成原生界面和本地数据层，提供今日、课表、查询、我的及相关详情页面，只读取真实 Repository。`verification/` 是独立数据验证宿主，不属于正式 App。

当前已完成界面对齐、按钮与抽屉样式整理，并通过 Debug 构建和 22 项单元测试；模拟器已核对未登录/认证过期状态下的深浅色及部分页面 1.3 倍字号。历史实测、真实样本限制及发布待办统一见[验证清单](docs/验证清单.md)，不以编译成功代替验收。
...
从[文档索引](docs/README.md)进入，按任务查看[开发指南](docs/开发指南.md)、[界面规范](docs/界面规范.md)、[接口参考](docs/接口参考.md)或[验证清单](docs/验证清单.md)。
