...
这里保留公开资料与脱敏的依赖核验证据。当前产品需求从[文档索引](../docs/README.md)阅读；详细设备记录和网页脚本快照只在本机保存。
...
本机网页脚本中可能同时存在写操作、其他角色或通用产品逻辑；它们不是首版需求，不得据此扩大范围。脚本快照属于学校网页的第三方代码，已退出 Git 跟踪；需要复核时从学校公开网页重新取得。设备采集与匿名入口记录也不随仓库发布。
