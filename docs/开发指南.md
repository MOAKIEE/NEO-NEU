...
更新日期：2026-09-25。本文描述功能范围与当前实现；界面约束见[界面规范](界面规范.md)，已验证与未验证范围见[验证状态](验证状态.md)。
...
下表是产品范围，不代表所有能力已实现；当前行为见后文，验收边界见[验证状态](验证状态.md)。
...
- 当前只接入门户消息并标注来源；将来接入教务消息时须分别标注，不合并为全校消息计数。本机阅读与学校已读分开，打开官方原文可能产生学校端访问或已读记录。消息富文本转为受控原生内容。
...
- 我的采用账号概览、偏好设置、数据与隐私及关于分组；主题选项和关于说明按需展开，退出入口合并为一个。标题栏使用 CampusTopBar，由 MainScreen 应用并消费 Scaffold 内边距，避免各页重复系统边距。账号显示通用“学校账号”，不推断身份、学历。当前没有独立“只清缓存”API，操作明确命名为“退出并清除本地数据”，说明会话和当前账号缓存都会清除；Miuix OverlayDialog 承担确认。

## 页面状态与适配

`SaveableStateHolder` 按主标签/详情路由隔离保存状态，筛选、查询词、学期 ID、周次、校区及列表模式使用可保存状态；滚动组件参与 Compose 保存机制。账号作用域变化时重建页面保存容器，避免旧作用域状态沿用。启动时复验已有会话；门户已连接而教务连接过期或暂不可达时，应用用共享的学校 Cookie 在后台打开教务官方入口，等待单点登录跳转后独立复验教务接口。用户点击教务连接入口时也先尝试这一步；无法静默恢复则打开可见的学校教务登录页。静默尝试不轮换账号作用域，可见的重新认证仍按原逻辑隔离缓存。官方登录页使用 Miuix 原生框架承载学校 WebView，分别显示门户和教务连接状态；用户完成网页登录后点击「完成登录，检查连接」，双域可用时自动返回应用，只有一域过期时打开对应的学校页面。未在登录页完成检查就返回时，主界面复验连接；登录页已确认双域可用时不重复发起探针。已连接的教务/门户触发对应数据恢复。
...
lifecycleScope.launch {
    ...
    campus.portal.refreshMessages(page = 1, pageSize = 30)
    val messages = campus.portal.messages(page = 1, pageSize = 30).value
...
`QuerySnapshot.data == null` 表示尚无成功数据；成功空列表是非 null 空集合。刷新失败可保留旧 `data`、原 `lastSuccessEpochMillis` 和 `isStale=true`。Room 只在完整解析成功后写入；账号作用域为本机随机 ID，退出清除当前范围与 WebView Cookie。每次进入学校登录页都会轮换本地作用域，以隔离可能的换号；已有会话的补登保留学校 Cookie 供单点登录使用，首次登录则先清理遗留 Cookie。更换学校账号需先在「我的」退出并清除本地数据，登录页也提示这一点；当前尚无可用于自动识别账号切换的可信身份字段。进程重启先标 `UNVERIFIED`，由 `session.verify()` 更新两域状态。
...
Room schema v1 位于 data/database/schemas/。旧 SQLite 已有一次迁移；后续版本必须提供显式 migration，不允许 destructive fallback。当前查询边界见[接口参考](接口参考.md)；实际白名单以 `SchoolCall` 为准。
