...
更新日期：2026-09-24。本文描述功能范围与当前实现；界面约束见[界面规范](界面规范.md)，已验证与未验证范围见[验证清单](验证清单.md)。
...
`QuerySnapshot.data == null` 表示尚无成功数据；成功空列表是非 null 空集合。刷新失败可保留旧 `data`、原 `lastSuccessEpochMillis` 和 `isStale=true`。Room 只在完整解析成功后写入；账号作用域为本机随机 ID，退出清除当前范围与 WebView Cookie。首次登录生成作用域；同一账号补登门户或教务时沿用该作用域，避免丢失另一域状态和本地缓存。更换学校账号需先在「我的」退出并清除本地数据，登录页也提示这一点；当前尚无可用于自动识别账号切换的可信身份字段。进程重启先标 `UNVERIFIED`，由 `session.verify()` 更新两域状态。
