...
`SaveableStateHolder` 按主标签/详情路由隔离保存状态，筛选、查询词、学期 ID、周次、校区及列表模式使用可保存状态；滚动组件参与 Compose 保存机制。账号作用域变化时重建页面保存容器，避免旧作用域状态沿用。官方登录页使用 Miuix 原生框架承载学校 WebView，分别显示门户和教务连接状态；用户完成网页登录后点击「完成登录，检查连接」，双域可用时自动返回应用，只有一域过期时打开对应的学校页面。未在登录页完成检查就返回时，主界面复验连接；登录页已确认双域可用时不重复发起探针。已连接的教务/门户触发对应数据恢复。
